...
impl<'model> Session<'model> {
    ...
    pub fn decode_step(&mut self, token: u32) -> &[f32] {
...

        {
            ...
            for dim_index in 0..embedding_dim {
                hidden_surface[dim_index * DECODE_SPATIAL_WIDTH] = self.model.weights.wte
                    [token_index * embedding_dim + dim_index]
        ...
        for (layer_index, layer) in self.model.executables.decode.iter().enumerate() {
            layer
                ...
                .run(
                    &[
                        &self.decode_hidden,
                        &self.kv_cache.keys[layer_index],
                        &self.kv_cache.values[layer_index],
                        &self.decode_mask,
                    ],
...

            {
                ...
                self.kv_cache.write_kv_from_attn(
                    layer_index,
                    &attn_slice,
...
fn embedding_lookup_into(
    ...
    for seq_index in 0..sequence_length {
        ...
        for dim_index in 0..embedding_dim {
            destination[dim_index * sequence_length + seq_index] = token_embeddings
                [token * embedding_dim + dim_index]
