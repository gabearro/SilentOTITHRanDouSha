...
fn layer_norm(
    ...
    let gamma_constant = graph.constant(
        gamma,
        Shape {
            batch: 1,
    ...
    let beta_constant = graph.constant(
        beta,
        Shape {
            batch: 1,
            channels: embedding_dim,
...
fn attention_body(
    ...
    let qkv_weight = graph.constant(
        &layer_weights.qkv_weight,
        Shape::spatial(3 * embedding_dim, 1, 1),
    );
    ...
    let key_new_flat = graph.slice(
        qkv,
        [0, embedding_dim, 0, 0],
        [1, embedding_dim, 1, query_seq],
    );
    let value_new_flat = graph.slice(
        qkv,
        [0, 2 * embedding_dim, 0, 0],
        [1, embedding_dim, 1, query_seq],
    );
    ...
    let projection_weight = graph.constant(
        &layer_weights.attn_proj_weight,
        Shape::spatial(embedding_dim, 1, 1),
    );
...
fn ffn_body(
    ...
    let normalized = layer_norm(
        graph,
        input,
        &layer_weights.ln2_weight,
        &layer_weights.ln2_bias,
        embedding_dim,
        config.layer_norm_epsilon,
    );

    let fc_weight = graph.constant(
        &layer_weights.fc_weight,
        Shape::spatial(4 * embedding_dim, 1, 1),
    );
